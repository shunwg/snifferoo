...
| Uten server (dobbeltklikk) | `dist/Ordforeren.html?fixture=07` — virker på `file://` |
...
| 25 | `SIGNIN` | Logg inn | Google/Apple foran nett-modusene. **Inert til `AUTH_CONFIG` er fylt ut** (`AUTH-SETUP.md`) — uten oppsett vises den aldri. Har alltid en vei ut til én telefon | [25](png/25-signin.png) |
...
| Sirene | Siste 5 sekunder: skjermkantene pulserer i bluffPink, ett mykt tofonet varsel og én vibrasjon idet den starter. Vises KUN når du fortsatt skylder et svar (09/11/08) — aldri på en venteskjerm | `ui.js sirenSet()` |
...
Neste ledige nummer er **26**. Ennå ikke bygget: **Pause/Innstillinger** (PRD §8). Den som bygger dem: legg til fixture i `fixtures.js`, gi neste ledige nummer her, kjør snap.
