...
The **web build only** (`Lab/`, `dist/Ordforeren.html`, GitHub Pages) gains three features. The iOS app in `Sources/` is untouched by this amendment and keeps every rule above.
...
- **Joining a game already in progress** *(amended 2026-07-27b, web build — supersedes the 3-minute window)*: the room accepts newcomers **for as long as the game is live**. Only a finished game (`phase: "winner"`) or a full room refuses; the joiner then lands back on the profile screen with the play-vs-bots way out. The three-minute door was the wrong shape for a party game — a session runs 20 minutes, people wander in, and a friend with the link should be able to sit down at minute eighteen instead of being told "we already started". This remains **not matchmaking**: the code or link is still required, so §2's "no matchmaking with strangers" holds and no moderation surface opens.

  There are exactly **two ways in**, and which one you get is decided by arithmetic, not preference:
  - **A bot's chair, immediately** — whenever a bot is sitting. They inherit its score, which the arrival banner states out loud so nobody thinks points appeared from nowhere. `players.length` does not move. Arriving this way after the card is drawn puts them in the per-round timed-out set: they watch that round out and play from the next, by the rule above.
  - **A seat at the next round** — when there is no bot to displace. They wait in the waiting room, watching the live round, and `newRound()` deals them in. A brand-new chair **may never appear mid-round**, because three engine functions read `playerCount` and all three change meaning if it moves underneath a round in flight: `gmForRound()` re-maps who is GM for every remaining round, `winCheck()` gates on `round % playerCount`, and `scoreRound()` sizes `Array(playerCount)` for the deltas. The waiting room the player experiences and the invariant the engine needs are the same rule.

  A newcomer taking a **fresh** chair starts **level with the current last place**, not at zero. Joining a 15-space game at round nine on zero is a spectator seat with extra steps — the table cannot be caught, so the game is over for them before their first word. The banner says so out loud, same as the inherited-bot-score case, and nobody already playing is overtaken by an arrival.

- **What someone watching, rather than playing, is shown** *(2026-07-27b, web build)*: a latecomer sees the live round — the word, the tick-in chips, the clock, the running tally — because a blank holding screen is a poor first impression of a game you were just invited to, and none of it helps them next round (that is a different word). But the waiting screens must **say which of the two they are**. Previously `WAIT` drew a green ✓ meaning *your lie is in* and `VOTEWAIT` said *du stemte*, to someone who had done neither and could not: an effect asserting a game state that is false, on the first screen a newcomer ever meets. A watcher gets a distinct spectator marker instead. Someone with no chair yet is never routed to an input screen, and the host never sends them the truth — `netProject()` treats "no seat" as "not the GM".
...
- [x] Name: **Ordføreren** — run a Patentstyret + EUIPO trademark search before any public App Store release (the name is deliberately phonetically adjacent to the Norwegian original; fine for TestFlight)
