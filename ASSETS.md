...
| `Lab/icon.svg` + `Lab/icon-180.png` + `Lab/icon-512.png` | Original — drawn in this repo | Ours | The rosette as an app icon, per DESIGN.md §4 Mark geometry: the same disc, tails and knocked-out centre as `LOGO()` and `.badge`, on the dark plane so it holds its edge against any home-screen wallpaper. Literal hexes rather than tokens, deliberately — an OS renders an icon outside any stylesheet. The SVG is the source of truth; the PNGs are generated by `node Tools/make-icons.mjs` (Safari has never accepted SVG for `apple-touch-icon`). Referenced by `Lab/manifest.webmanifest` and base64-inlined into the single-file build |
